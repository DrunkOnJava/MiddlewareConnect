...
extension UIApplication {
    ...
    func registerBackgroundTasks() {
        ...
        BGProcessingTaskRequest.register(forTaskWithIdentifier: "com.llmbuddy.backgroundProcessing") { task in
            task.earliestBeginDate = Date(timeIntervalSinceNow: 15 * 60)
            return true
        }
        
        // Register a background refresh task
        BGAppRefreshTaskRequest.register(forTaskWithIdentifier: "com.llmbuddy.backgroundRefresh") { task in
            task.earliestBeginDate = Date(timeIntervalSinceNow: 60 * 60)
            return true
        }
    }
}